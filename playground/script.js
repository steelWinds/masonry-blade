...
function updateMetrics() {
	...
	elements.recreateMetric.textContent = formatDuration(
		state.recreateDurationMs,
	);
...
function updateControls() {
	...
	elements.responsiveButton.textContent = state.responsiveEnabled
		? 'Disable Responsive'
		: 'Enable Responsive';
	elements.responsiveButton.setAttribute(
		'aria-pressed',
		String(state.responsiveEnabled),
	);
}
...
function getRequestedColumns() {
	return state.responsiveEnabled
		? getResponsiveColumns()
		: Number(elements.columnSelect.value);
}
...
function createMatrix(columnCount) {
	...
	state.matrix = new MasonryMatrix(width, columnCount);
...
async function appendItems(batchItems, sessionId) {
	...
	state.loadedItems = columns.flat().length - 1;
	render(columns);
	updateMetrics();
}
...
async function recreateMatrix(columnCount, sessionId) {
	...
	const startedAt = performance.now();
	const columns = await enqueue(() =>
		matrix.recreateMatrix(getMatrixRootWidth(columnCount), columnCount),
	);
...
function handleResize() {
	...
	state.resizeTimeoutId = window.setTimeout(() => {
		...
		recreateMatrix(nextColumns, sessionId).catch((error) => {
			const message =
				error instanceof Error ? error.message : 'Unknown recreate error';
			setStatus(message, 'error');
...
function bindEvents() {
	...
	elements.columnSelect.addEventListener('change', () => {
		...
		setStatus(
			'New amount selected. Please click to Reload, for recreate matrix.',
		);
	});
}
